...
- [x] 일정 엔진: 공종 소요일 자동 산정 + 선후행 + 예비/비상 자동 추가
- [x] 간트차트 UI(드래그 수정, 선행 위반 경고) + 일정표 PDF
- [x] 문자 발송: NHN Cloud SMS or 카카오 알림톡 연동(둘 중 빠른 승인 우선)
- [x] 문자 템플릿(작업자: 주소+날짜+비번+작업내용 / 고객: 일정·진행) + 미리보기 + 멱등성 + 비번 마스킹 로그
- [x] AI 게이트웨이(재시도/타임아웃/비용로깅 `ai_invocations`)
- [x] 홈 "오늘 할 일" 대시보드
- [x] PWA 설치 가이드(태블릿 홈 화면 추가), 대형 폰트/버튼 접근성 점검
- [x] 실사용 온보딩: 단가표 입력 마법사

### Phase 2 — 운영 도구 (CRM·작업자·재무·사진) · 약 6~8주
**목표**: 흩어진 데이터를 한곳에 모으고 운영을 자동화.
**완료 기준**: 연락처 CSV 수백 건 가져오기, 현장별 마진 확인, 사진 자동 분류가 동작.
- [x] 고객 CRM + 연락처 CSV 가져오기(중복 병합 UI) + 메모 디지털화
- [x] AI 우수고객 등급 분류(공사 횟수/매출/최근성 기반, 사람 확정) + 주기 인사 문자 스케줄
- [x] 작업자/업체 DB + 공종·평점·이력 + 가용일정·배정 + 예약 문자 자동
- [x] 재무: 현장별/업체별 입출금 집계, 마진율, 월·연 대시보드
- [x] AI 사진 관리: 업로드→EXIF 날짜 분류→Claude Vision 태깅 원클릭 + 수동 보정

### Phase 3 — 성장 기능 (인스타·자재산출·무드보드) · 약 6주
**목표**: 마케팅·정밀화로 매출 기여.
**완료 기준**: 인스타 자동 게시(확정 게이트), 면적→자재수량, 무드보드 생성.
- [x] 인스타: Vision 사진 선별 → 캡션 생성 → Graph API 게시(비즈계정·확정 후 게시)
- [x] 자재 수량 산출: 공식 테이블 입력 + 면적→수량 계산
- [x] 3D 무드보드: gpt-image-1, "참고용" 워터마크/고지 강제

### Phase 4 — SaaS 전환 · 약 6~8주
**목표**: 다수 사업자 셀프 온보딩 판매.
**완료 기준**: 신규 사업자가 결제→온보딩→첫 견적까지 자력 완료.
- [x] 멀티테넌트 본격화(가입 플로우, 테넌트 프로비저닝, RLS 부하·격리 검증)
- [x] 요금제(basic/pro/team) + 결제(토스페이먼츠/스트라이프) + 기능 게이팅
- [x] 온보딩 마법사(단가표 가이드·데이터 가져오기) + 도움말/튜토리얼
- [x] 사용량·비용 모니터링(테넌트별 AI 비용), 관리자 콘솔
