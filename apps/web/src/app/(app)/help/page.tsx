  ...
  return (
    <div className="pb-24">
      <header className="sticky top-0 bg-card border-b border-border z-10 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-foreground flex-1">도움말</h1>
      </header>

      <div className="px-4 pt-6 space-y-6">
      <p className="text-lg text-muted-foreground">궁금한 메뉴를 눌러보세요</p>

      {/* 빠른 이동 */}
      <section>
        <h2 className="text-xl font-bold text-foreground mb-3">바로 이동</h2>
        <div className="grid grid-cols-2 gap-2">
          {(
            ...
            >
              ...
              <span className="text-base font-semibold text-foreground flex-1">{item.label}</span>
              <ChevronRightIcon size={16} className="text-muted-foreground/50 shrink-0" />
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* 사용법 */}
      <section>
        <h2 className="text-xl font-bold text-foreground mb-3">사용법 안내</h2>
        <div className="space-y-3">
          {keys.map((key) => {
            ...
            return (
              <details
                key={key}
                className="bg-card border border-border rounded-2xl px-5 [&_summary::-webkit-details-marker]:hidden"
              >
                <summary className="flex items-center gap-3 py-4 cursor-pointer text-lg font-bold text-foreground list-none">
                  <span className="text-2xl">{content.icon}</span>
                  <span>{content.title}</span>
                  <span className="ml-auto text-muted-foreground/70 text-base">자세히 ▼</span>
                </summary>
      ...
      {/* 문의 */}
      <section className="bg-muted border border-border rounded-2xl px-5 py-5 text-center">
        <p className="text-lg font-bold text-foreground mb-1">더 궁금한 게 있으신가요?</p>
        <p className="text-base text-muted-foreground">앱을 사용하다 막히면 언제든 물어보세요</p>
      </section>
      </div>
    </div>
  );
