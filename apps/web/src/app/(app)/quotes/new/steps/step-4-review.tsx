...
export function Step4Review({ siteId, siteName, items, distanceFactor, difficultyFactor, areaPyeong, onConfirmed, onBack }: Props) {
  ...
  return (
    <div className="px-4 pt-6">
      <h2 className="text-2xl font-bold text-foreground mb-2">견적 확인</h2>
      <p className="text-lg text-muted-foreground mb-6">금액을 확인하고 확정하세요</p>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden mb-4">
        {result.items.map((item) => (
          <div key={`${item.tradeId}-${item.description}`} className="px-4 py-3 border-b border-border last:border-0">
      ...
      <div className="bg-muted border border-border rounded-xl p-4 mb-6">
        ...
        <div className="flex justify-between items-baseline border-t border-border pt-3">
          <span className="text-xl font-bold text-foreground">합계</span>
      ...
      <div className="flex gap-3">
        <button
          onClick={onBack}
          className="flex-1 py-4 text-lg border border-border rounded-xl text-muted-foreground font-medium active:bg-muted"
        >
          ◀ 다시 볼게요
        </button>
        <button
          onClick={() => setShowConfirmDialog(true)}
          className="flex-1 py-4 text-lg bg-primary text-white rounded-xl font-bold shadow-primary-glow active:scale-[0.98] transition-all duration-200"
        >
          이 금액으로 확정
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-card rounded-2xl p-6 w-full max-w-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-xl font-bold text-foreground">견적을 확정할까요?</h3>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={isPending}
                className="flex-1 py-4 text-lg border border-border rounded-xl text-muted-foreground active:bg-muted"
              >
                아니요
              </button>
              <button
                ...
                disabled={isPending}
                className="flex-1 py-4 text-lg bg-primary text-white rounded-xl font-bold shadow-primary-glow active:scale-[0.98] disabled:shadow-none disabled:opacity-50 transition-all duration-200"
              >
                {isPending ? "확정 중..." : "네, 확정"}
              </button>
            </div>
